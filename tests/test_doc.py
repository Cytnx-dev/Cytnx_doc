...
def excute_all(test_names, capsys):
    ...
    for test_name in test_names:
        code = open(test_codes_path + test_name + ".py").read() + get_redirect_code(
            test_name
        )
        codes = codes + code
    exec(codes)
...
def test_guide_basic_obj_Tensor_8_cp_permute(capsys):
    test_names = [
        "guide_basic_obj_Tensor_8_cp_permute-1",
...
def test_guide_basic_obj_Tensor_8_cp_contiguous(capsys):
    test_names = [
        "guide_basic_obj_Tensor_8_cp_contiguous-1",
...
def test_guide_uniten_create_complex(capsys):
    test_names = ["guide_uniten_create_complex", "guide_uniten_create_print_diagram"]
    excute_all(test_names, capsys)
...
def test_guide_uniten_labels_relabel(capsys):
    test_names = ["guide_uniten_labels_relabel_", "guide_uniten_labels_relabel"]
    excute_all(test_names, capsys)
...
def test_guide_uniten_tagged(capsys):
    with pytest.raises(Exception) as e_info:
        test_names = ["guide_uniten_tagged_init", "guide_uniten_tagged_contract"]
        excute_all(test_names, capsys)
...
def test_guide_uniten_elements_sym(capsys):
    with pytest.raises(Exception) as e_info:
        test_names = [
            "guide_uniten_elements_init_sym",
...
def test_guide_uniten_elements_exists(capsys):
    test_names = ["guide_uniten_elements_init_sym", "guide_uniten_elements_exists"]
    excute_all(test_names, capsys)
...
def test_guide_contraction_network_launch(capsys):
    test_names = [
        "guide_contraction_network_PutUniTensor",
...
def test_guide_contraction_network_label_ord(capsys):
    test_names = [
        "guide_contraction_network_label_ord-1",
...
def test_guide_itersol_LinOp_matvec(capsys):
    test_names = ["guide_itersol_LinOp_matvec", "guide_itersol_LinOp_demo"]
    excute_all(test_names, capsys)
...
def test_guide_xlinalg_Svd(capsys):
    test_names = ["guide_xlinalg_Svd", "guide_xlinalg_Svd_verify"]
    excute_all(test_names, capsys)
